...
class MySolutionBFS:  # TLE case 31
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        while transformation:
            newtrans = set()
            depth += 1
            for word in transformation:
                if word == endWord:
                    return depth

                for i in range(n):
                    for exp in wordSet.copy():
                        if word[:i] == exp[:i] and word[i + 1:] == exp[i + 1:]:
                            newtrans.add(exp)
                            wordSet.remove(exp)
...
class Solution_birdirectional_BFS(object):
    def __init__(self):
        self.length = 0
        # Dictionary to hold combination of words that can be formed,
        # from any given word. By changing one letter at a time.
